...
The primary dataset for the final deliverable is `data/sample/works.jsonl`, a deterministic synthetic catalog generated by `scripts/generate_synthetic_data.py`. This is intentional: StorySeek's schema requires fanfiction-style metadata such as tropes, relationship dynamics, audience rating, and content warnings. Public-domain book catalogs usually provide title, author, subjects, and language, but not this trope-aware metadata.
...
Fields such as `themes`, `tropes`, `relationship_dynamics`, and `content_warnings` would need manual labels or conservative rule-based tags, and should be documented as incomplete.
