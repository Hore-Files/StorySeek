...
For the current Gutenberg report, hybrid has the strongest recall because it fuses candidates from both lexical and dense retrieval. Dense has slightly stronger mean nDCG than BM25 on the current LLM-assisted labels, but the small query count and AI-assisted judgments should be disclosed.
